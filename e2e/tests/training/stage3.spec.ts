    ...
    test('загрузка и запуск этапа 3 тренировки', async ({ page }) => {
        ...
        if (buttonVisible) {
            ...
            if (isEnabled) {
                ...
                // Кликаем на этап 3 в селекторе этапов
                await trainingPage.clickStage(3);
                await page.waitForTimeout(1000); // Ждем переключения этапа

                // Проверяем, что этап 3 отображается
                await trainingPage.expectStage(3);

                // Проверяем заголовок этапа 3
                await trainingPage.expectStageTitle('Сопоставление слов');
            }
        }
    });
});
