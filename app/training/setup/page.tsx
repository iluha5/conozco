...
export default function TrainingSetupPage() {
    ...
    const {
        selectedLanguage,
        setSelectedLanguage,
        isLoaded: selectionLoaded,
    } = useTrainingSelection();
    const { selectedWords, setSelectedWords } = useTrainingWords();
    const { settings: stage1Settings, updateSettings: updateStage1 } =
        useStage1Settings();
    const { settings: stage4Settings, updateSettings: updateStage4 } =
        useStage4Settings();
    const { settings: stage5Settings, updateSettings: updateStage5 } =
        useStage5Settings();

    const [words, setWords] = useState<Word[]>([]);
    const [visibleWordsCount, setVisibleWordsCount] = useState(12);
    const [isInitialSelection, setIsInitialSelection] = useState(true);
    const [showStage1SettingsModal, setShowStage1SettingsModal] =
        useState(false);
    const [showStage4SettingsModal, setShowStage4SettingsModal] =
        useState(false);
    const [showStage5SettingsModal, setShowStage5SettingsModal] =
        useState(false);
                            ...
                            <div>
                                ...
                                {!isLoadingWords && (
                                    <p className="text-xs text-gray-500 mt-2">
                                        ...
                                        из {getFilteredWords().length} слов
                                    </p>
                                )}
                            </div>
                                                ...
                                                {(stage === 1 ||
                                                    ...
                                                    <Button
                                                        ...
                                                        onClick={e => {
                                                            e.stopPropagation();
                                                            if (stage === 1)
                                                                setShowStage1SettingsModal(
                                                                    true,
                                                                );
                                                            if (stage === 4)
                                                                setShowStage4SettingsModal(
                                                                    true,
                                                                );
                                                            if (stage === 5)
                                                                setShowStage5SettingsModal(
                                                                    true,
                                                                );
                                                        }}
