...
export const getTrainingSettings = (userId: string): TrainingSettings => {
  ...
  try {
    const savedSettings = localStorage.getItem(getStorageKey(userId, 'settings'))
    if (savedSettings) {
 ...
 */
export const saveTrainingSettings = (userId: string, settings: TrainingSettings): void => {
  if (typeof window === 'undefined') {
    return
  }
 ...
 */
export const saveStage1Settings = (userId: string, stage1Settings: Stage1Settings): void => {
  const settings = getTrainingSettings(userId)
 ...
 */
export const saveStage4Settings = (userId: string, stage4Settings: Stage4Settings): void => {
  const settings = getTrainingSettings(userId)
 ...
 */
export const saveStage5Settings = (userId: string, stage5Settings: Stage5Settings): void => {
  const settings = getTrainingSettings(userId)
...
export const getSelectedWords = (userId: string): string[] => {
  ...
  try {
    const saved = localStorage.getItem(getStorageKey(userId, 'selected-words'))
    if (saved) {
...
export const saveSelectedWords = (userId: string, wordIds: string[]): void => {
  ...
  try {
    localStorage.setItem(getStorageKey(userId, 'selected-words'), JSON.stringify(wordIds))
  } catch (error) {
...
export const getSelectedLanguage = (userId: string): string => {
  ...
  try {
    const saved = localStorage.getItem(getStorageKey(userId, 'selected-language'))
    if (saved) {
      return saved
    }
  } catch (error) {
 ...
 */
export const saveSelectedLanguage = (userId: string, language: string): void => {
  if (typeof window === 'undefined') {
    return
  }

  try {
    localStorage.setItem(getStorageKey(userId, 'selected-language'), language)
  } catch (error) {
