...
export function TranslationDisplay({
    ...
    return (
        <div className="space-y-4 sm:space-y-6">
            {/* Блок перевода */}
            <div className="bg-purple-50/30 border border-purple-200 rounded-xl px-4 py-4 sm:px-6 sm:py-5 text-center">
                <p className="text-xl sm:text-2xl md:text-3xl text-purple-600 font-medium break-words">
            ...
            {filteredExamples.length > 0 && (
                <div className="bg-gray-50/50 rounded-xl px-4 py-4 sm:px-6 sm:py-5">
                    <div className="space-y-3 sm:space-y-4">
                        {filteredExamples.map((example, exampleIndex) => (
                            <div
                                key={exampleIndex}
                                className="border-l-4 border-purple-300 pl-3 sm:pl-4"
                            >
                                <p className="text-sm sm:text-base text-gray-700 mb-1 break-words">
                                    {example.example}
                                </p>
                                <p className="text-xs sm:text-sm text-purple-500 italic break-words">
                                    {example.translation}
                                </p>
                            </div>
                        ))}
